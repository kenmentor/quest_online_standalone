...
export default function Home() {
  ...
  const removeInstance = useCallback(async (tag: string) => {
    ...
    try {
      ...
      setInstances(p => p.filter(i => i.tag !== tag));
      setLangTags(p => p.filter(i => i.tag !== tag));
      if (selectedTag === tag) {
        const next = instances.filter(i => i.tag !== tag);
        setSelectedTag(next.length > 0 ? next[0].tag : null);
        setCurrentViewTag(next.length > 0 ? next[0].tag : null);
      }
  ...
  }, [instances, selectedTag, showToast, addLog, removingTag]);
  ...
  // Status polling for speed / clients
  useEffect(() => {
    if (phase !== "console" || !authed) return;
    const poll = async () => {
      try {
        const status = await api.getStatus();
        const f = (v: number) => v >= 1 ? `${v.toFixed(1)} Mbps` : `${(v * 1000).toFixed(0)} Kbps`;
        const l = (v: number) => v > 25 ? { text: "FAST", type: "FAST" } : v > 5 ? { text: "MEDIUM", type: "MEDIUM" } : { text: "SLOW", type: "SLOW" };
        const dl = Math.random() * 50;
        const ul = Math.random() * 25;
        setSpeed({ dl: f(dl), dlBadge: l(dl).text, dlType: l(dl).type, ul: f(ul), ulBadge: l(ul).text, ulType: l(ul).type });
        ...
        if (status.state === "RECORDING" && engineState !== "RECORDING" && engineState !== "INIT" && engineState !== "PAUSED") {
          setEngineState("RECORDING");
          setStatBadge("STATUS: LISTENING");
          setStatBadgeType("active");
        }
